...
//   reader->AddVariable( "vtxchi2",&vtxchi2_c);
   ...
   TString weightfile = dir + TString("TMVAClassification_BDT_vtx.weights.xml");
   reader->BookMVA( methodName, weightfile );
}

void add_mva_vtx(string treepath = "tagnprobe_input_era5.root",TString treename = "sig_tree", TString mvaname = "mva2") {
...
//   string dirIn="/eos/user/w/wangz/darkphoton/";
   ...
   for(unsigned int f=0; f<filesToRun.size(); f++){
    ...
    TTree *tree = (TTree*)fin->Get("tpTree/" + treename);
    ...
    TBranch *mumva2 = tree->Branch(mvaname,&mva2,"mva2/F");
//    tree->Branch("mva",mva); 
